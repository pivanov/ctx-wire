...
func TestRegistryFind(t *testing.T) {
	...
	tests := []struct {
		...
		{"gcc-13 (versioned) matches", "gcc-13 -c foo.c", "gcc"},
		{"clang matches (same diagnostics as gcc)", "clang -Wall -c foo.c", "gcc"},
		{"clang++ matches", "clang++ -std=c++20 main.cpp", "gcc"},
		{"clang-17 (versioned) matches", "clang-17 -c foo.c", "gcc"},
		{"cc matches", "cc -c foo.c", "gcc"},
		{"c++ matches", "c++ main.cpp", "gcc"},
		{"clangd does NOT match (language server, not a compiler)", "clangd --check=foo.c", ""},
		{"clang-format does NOT match (formatter)", "clang-format -i foo.c", ""},
		{"clang-tidy does NOT match (linter)", "clang-tidy foo.c", ""},
		{"c++filt does NOT match (demangler)", "c++filt _Z3foov", ""},
		{"ccache wrapper does not match", "ccache gcc -c foo.c", ""},
		{"gcc-ar does NOT match (toolchain archiver wrapper)", "gcc-ar rcs lib.a foo.o", ""},
		{"gcc-nm does NOT match (toolchain symbol lister)", "gcc-nm lib.a", ""},
		{"gcc-ranlib does NOT match (toolchain index wrapper)", "gcc-ranlib lib.a", ""},
		{"g++-ar does NOT match (toolchain archiver wrapper)", "g++-ar rcs lib.a foo.o", ""},
		...
		{"git stash show matches (diffstat)", "git stash show --stat", "git-diff"},
		{"git stash show -p matches", "git stash show -p stash@{0}", "git-diff"},
		{"git stash pop does NOT match", "git stash pop", ""},
		...
		{"npx @biomejs/biome routes to biome (scoped pkg, not node-package)", "npx @biomejs/biome check .", "biome"},
		{"npx markdownlint-cli routes to markdownlint", "npx markdownlint-cli .", "markdownlint"},
		{"npx oxlint routes to oxlint", "npx oxlint .", "oxlint"},
		{"npx unknown tool stays node-package", "npx some-random-tool", "node-package"},
		...
		{"next build matches next-build", "next build", "next-build"},
		{"local-bin next build matches next-build", "./node_modules/.bin/next build", "next-build"},
		{"npx next build stays node-package (not next-build)", "npx next build", "node-package"},
		{"next dev stays passthrough (must stream, not collapse)", "next dev", ""},
		...
		{"rubocop matches", "rubocop", "rubocop"},
		{"bundle exec rubocop matches", "bundle exec rubocop", "rubocop"},
		{"bin/rubocop binstub matches", "bin/rubocop", "rubocop"},
