...
| `ctx-wire rewrite [--json] [--agent <name>] <line>` | Print the rewritten form of a shell command line; `--json` includes per-segment metadata for integrations |
...
| `ctx-wire telemetry [status\|preview\|enable\|disable]` | Show anonymous aggregate telemetry status; `disable` drops only the per-command breakdown used to tune filters, aggregate website stats stay on, and `preview` prints the exact payload |
